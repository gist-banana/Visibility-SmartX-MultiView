	...
	public String getBoxStatus(String serverMgmtIp, String serverDataIp, String command, String usernameString,
		...
		try {
			...
			if (br.readLine() != null) {
				...
				// System.out.println("Box : "+serverMgmtIp+" Data Interface Status:
				// "+InterfaceStatus);
				ch.ethz.ssh2.Session sess2 = conn.openSession();
				sess2.execCommand(
						"ovs-ofctl show brcap | grep vxlan | cut -f 1 -d : | cut -f 2 -d '(' | cut -f 1 -d ')'");
				...
				while (true) {
					...
					if (host != null) {
						...
						if (type.equals("B**"))
							sess3.execCommand("ping -c 1 `ovs-vsctl show | grep -A2 " + host
						...
						if (m_status_new.equalsIgnoreCase("UP")) {
							...
							if (ovsVM_USER == null) {
								d_status = getBoxStatus(m_ip, d_ip,
										"netstat -ie | grep 'inet addr:" + d_ip + "' | cut -f 2 -d :", SmartXBox_USER,
								...
								if (boxtype.equals("B**")) {
									...
									d_status = getBoxStatus(activeVM.equals("ovs-vm1") ? ovsVM1ip : ovsVM2ip, d_ip,
											"netstat -ie | grep 'inet addr:" + d_ip + "' | cut -f 2 -d :", ovsVM_USER,
								...
								// brcap Inside Box
									...
									// System.out.println("Box: "+ box + " Type: "+boxtype+" Active VM"+ activeVM);
									d_status = getBoxStatus(m_ip, d_ip,
											"netstat -ie | grep 'inet addr:" + d_ip + "' | cut -f 2 -d :",
											SmartXBox_USER, SmartXBox_PASSWORD, boxtype);
								}
							}

							/*
							 * } else { m_status_new="RED"; d_status="RED"; }
							 */
						} else {
							m_status_new = "RED";
							d_status = "RED";
