...
public class BridgesVLANMapping implements Runnable {
	...
	private String SmartXBox_USER, SmartXBox_PASSWORD, OVS_VM_USER, OVS_VM_PASSWORD, box = "", m_ip = "",
			boxStatus = "", ovsVM1ip, ovsVM2ip, activeVM;
	...
	public BridgesVLANMapping(String boxUser, String boxPassword, String ovsvmuser, String ovsvmpwd,
			MongoDB_Connector MongoConn, String pboxCollection, String vlanmapCollection, String vlanmapCollectionRT,
			int vlanstart, int vlanend, String[] boxType) {
	...
	public void getBridgesStatus(String box, String serverIp, String bridgeName, String command, String usernameString,
			String password) {
		try {
			...
			while (true) {
				...
				if (line != null) {
					...
					if ((!line.contains("actions=drop")) && line.contains("dl_vlan") && !line.contains("actions=strip")
							&& !line.contains("actions=mod_dl_src")) {
	...
	public void setBridgesStatus(String box, String serverIp, String bridgeName, String result) {
		...
		if (Integer.parseInt(dl_vlan) >= VLAN_START && Integer.parseInt(dl_vlan) <= VLAN_END) {
			...
			LOG.debug("[" + dateFormat.format(timestamp) + "][INFO][Bridge VLAN][Box: " + box + " Bridge: " + bridgeName
					+ " VLAN: " + dl_vlan + " inserted]");
		}
	}
		...
		pBoxList.forEach(new Block<Document>() {
			public void apply(final Document document) {
				...
				if (boxStatus.equals("GREEN")) {
					...
					try {
						getBridgesStatus(box, m_ip, "br-int", "ovs-ofctl dump-flows br-int | grep dl_vlan",
								SmartXBox_USER, SmartXBox_PASSWORD);
						getBridgesStatus(box, m_ip, "brvlan", "ovs-ofctl dump-flows brvlan | grep dl_vlan",
								SmartXBox_USER, SmartXBox_PASSWORD);
						getBridgesStatus(box, activeVM, "brdev", "sudo ovs-ofctl dump-flows brdev | grep dl_vlan",
								OVS_VM_USER, OVS_VM_PASSWORD);
